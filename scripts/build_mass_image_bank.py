...
def load_manifest(data_dir: Path):
    full = data_dir / 'hg-v12.full.xz'
    if full.is_file():
        try:
            return json.loads(lzma.decompress(full.read_bytes()).decode('utf-8'))
        except Exception as exc:
            raise SystemExit(f'Could not decode {full}: {exc}')

    # Compatibility fallback for the temporary multipart snapshot used during development.
    parts = sorted(data_dir.glob('hg-v12.part*.b64'))
    if not parts:
        raise SystemExit(f'No migration manifest found in {data_dir}')
    payload = ''.join(p.read_text(encoding='utf-8').strip() for p in parts)
    try:
        return json.loads(lzma.decompress(base64.b64decode(payload)).decode('utf-8'))
    except Exception as exc:
        raise SystemExit(f'Could not decode multipart migration manifest: {exc}')
